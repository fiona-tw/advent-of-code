...
def run_day_1():
    raw_calibration_document = _get_input_for_day(1)
    print(sum_of_calibration_values(raw_calibration_document))


def run_day_2():
    raw_games = _get_input_for_day(2)
...
def run_for_day_x(day: int):
    try:
        ...
        print(f"No solution implemented for day {day}.\t(`run_day_{day}` does not exist!)")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Please supply the day as a number for the puzzle you want to run\n\t('{day}' is not an integer!)")
    else:
        print(f"\nDay {day} solution:\n")
        run_for_day_x(day)
